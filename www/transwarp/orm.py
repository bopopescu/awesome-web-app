...
'''
database operation modules. this module is independent with web module
...
_triggers = frozenset(['pre_insert','per_update','per_delete'])
...
class ModelMetaclass(type):
	...
	#store all subclass info:
		...
		logging.info('Scan ORMapping...'% name)
		mappings =dict()
		primary_key = None
		for k,v in attrs.iteritems():
			...
			attrs['__sql__'] = lambda self: __gen_sql(attrs['__table'],mappings)
...
class Model(dict):
	...
	def count_by(cls,where,*args):
		return db.select_int('select count(`%s`) from `%s` %s'%(cls.__primary_key__.name,clas.__table___,where,)*args)
	@classmethod
	def update(self):
	...
	def delete(self):
		...
		db.update('delete from `%s` where `%s`=?'%(self.__table,pk),*args)
		return self
